...
: m_active(0),
  ...
  m_fcp(100.0f)
{
	Camera *cam1 = new Camera(vec3(0, 4, 8), 0, 0, 0, m_fov, m_ncp, m_fcp);
...
void CameraManager::renderCameras(const Transform &trans, const GlobalObjectParam &param)
{
	if(!m_useCam)
	{
		for(uint i=0; i<m_cameras.size(); ++i)
		{		
            m_cameras[i]->update();
			m_cameras[i]->render(trans, param);
		}
	}
}
...
void CameraManager::currentCamParams(GlobalObjectParam &param)
{
    if(m_useCam)
    {
    ...
    else
    {
        ...
        param.camPos = vec3(camPos.x, camPos.y, camPos.z);
    }
}